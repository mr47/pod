...
var conf         = require('./conf'),
    ...
    pathNormalize   = require('./win'),
    hookTemplate = fs.readFileSync(__dirname + '/../hooks/post-receive', 'utf-8')

// Load config data
var globalConfigPath = pathNormalize(conf.path),
...
// mostly for testing.
if (process.env.POD_ROOT_DIR) globalConfig.root = pathNormalize(process.env.POD_ROOT_DIR);

// create default folders
if (!fs.existsSync(globalConfig.root)) mkdirp.sync(pathNormalize(globalConfig.root))
if (!fs.existsSync(pathNormalize(globalConfig.root + '/apps'))) fs.mkdirSync(pathNormalize(globalConfig.root + '/apps'))
if (!fs.existsSync(pathNormalize(globalConfig.root + '/repos'))) fs.mkdirSync(pathNormalize(globalConfig.root + '/repos'))
...
function getAppMainScript (workPath, appname) {
    ...
    if (main) {
        if (/\.js$/.test(main)) {
            ...
            var mainPath = pathNormalize(path.resolve(workPath, main))
...
function readJSON (file) {
    if (!fs.existsSync(file)) {
        ...
        try {
            ...
            var result = fs.readFileSync(file, 'utf-8').toString().replace(/\\/g,"\\\\");
